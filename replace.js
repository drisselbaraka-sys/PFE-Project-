...
const path = 'Frontend/src/components/CreateCenter.jsx';
let text = fs.readFileSync(path, 'utf8');

const aiConfigMatch = text.match(/(\{step === 'ai_config' && !isLoading && \(\s*<div\s+className="flex-1 flex flex-col items-center p-4 md:p-8 overflow-y-auto custom-scrollbar font-inter transition-colors duration-500[\s\S]*?\{step === 'ai_config' && isLoading && \()/);

if (!aiConfigMatch) {
    console.error("ai_config_match failed");
    process.exit(1);
}
const aiConfigPart = aiConfigMatch[1];
const col1 = aiConfigPart.match(/(<div className="space-y-6 glass-card p-6 rounded-\[32px\] shadow-2xl transition-all duration-500"[\s\S]*?{\/\* Column 2: Source \(Content & Files\) \*\/})/)[1];
const col3 = aiConfigPart.match(/({\/\* Column 3: AI Settings \(Parameters\) \*\/}[\s\S]*?<\/div>\s*<\/div>\s*{\/\* Footer Controls \*\/})/)[1];


const manualEditorPart = "{step === 'manual_editor' && (" + text.split("{step === 'manual_editor' && (")[1].split("{step === 'success' && (")[0];

let col1clean = col1.replace("{/* Column 2: Source (Content & Files) */}", "");
let col3clean = col3.replace("{/* Footer Controls */}", "");
...
const newManualEditor = `{step === 'manual_editor' && (
...
${col1clean}${manualCol2}${col3clean}${footer}`;

text = text.replace(manualEditorPart, newManualEditor + "\n                ");
